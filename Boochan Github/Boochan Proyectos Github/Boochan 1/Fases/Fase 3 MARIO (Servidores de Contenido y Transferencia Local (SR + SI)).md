...
- **3. Implementación de Cifrado SSL/TLS (FTPS)**
	...
	```
	# Quitar '#' en TLSEngine para activar el motor 
	TLS TLSEngine on 
	
	# Quitar '#' en TLSLog para registrar intentos de conexión segura 
	TLSLog /var/log/proftpd/tls.log 
	
	# Quitar '#' en TLSProtocol para definir la compatibilidad de protocolos 
	TLSProtocol SSLv23 
	
	# Quitar '#' y añadir la ruta al archivo .crt generado 
	TLSRSACertificateFile /etc/ssl/certs/proftpd.crt 
	
	# Quitar '#' y añadir la ruta a la clave privada .key 
	TLSRSACertificateKeyFile /etc/ssl/private/proftpd.key
	
	# Añadir '#' en TLSRSNRequired para desactivar requerimientos de nombre de sitio 
	# TLSRSNRequired off 
	
	# Añadir '#' en TLSRenderCondition para evitar conflictos de renderizado SSL
	# TLSRenderCondition standard
	```
