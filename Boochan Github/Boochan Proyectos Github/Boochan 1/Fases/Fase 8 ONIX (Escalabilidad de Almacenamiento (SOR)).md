...
- **Ampliacion del disco:** En VirtualBox, vamos a Archivo -> Herramientas -> Media -> Propiedades -> Seleccionamos el disco que querramos ampliar y aumentamos el espacio hasta **100 GB** (En caso de error ir hasta el final de la fase)
...
En caso de error, será muy posiblemente porque sea un disco estático en vez de dinámico, si es el caso realizaremos estos pasos:

1. Nos iremos a la CMD
2. Ubicaremos donde esta instalado VirtualBox, en concreto el VBoxManager.exe
3. Una vez ubicado, haremos un cd a la carpeta donde se encuentre el .exe y pondremos este comando a continuación: 
   ```
   .\VBoxManage.exe clonemedium "C:\Users\(VuestroUsuario)\VirtualBox VMs\(VuestraVM)\(VuestraVM).vdi" "C:\Users\(VuestraVM)\VirtualBox VMs\(VuestraVM)\(VuestraVM).vdi" --variant Standard
   ```
